#!/usr/bin/env python3
"""
AUTOMATED TEST SUITE: Void Protection System (No User Input)

Validates all functionality automatically without user prompts.
"""
...
class VoidSystemTester:
    ...
    def test_different_models(self) -> Tuple[bool, float, float]:
        ...
        try:
            ...
            # Trigger attacker detection with systematic queries
            for _ in range(15):
                requests.post(
                    f"{self.server_url}/api/risk",
            ...
            # Query as attacker (should now be detected)
    ...
    def run_all_tests(self):
        ...
        if not self.test_server_running():
            ...
            print("  Command: python demo_server.py")
            return False
...
║                                                                      ║
║  This script validates all components automatically.                 ║
║  NO USER INPUT REQUIRED - Tests run immediately.                     ║
║                                                                      ║
╚══════════════════════════════════════════════════════════════════════╝
    """)
